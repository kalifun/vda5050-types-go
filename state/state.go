...
type State struct {
	...
	ZoneSetId             *string       `json:"zoneSetId,omitempty"`             // Optional unique ID of the zone set
	...
	AgvPosition           *AgvPosition  `json:"agvPosition,omitempty"`           // Current position of the AGV on the map (optional)
	...
	Paused                *bool         `json:"paused,omitempty"`                // Indicates if AGV is paused (optional)
	NewBaseRequest        *bool         `json:"newBaseRequest,omitempty"`        // Request new base from master (optional)
	DistanceSinceLastNode *float64      `json:"distanceSinceLastNode,omitempty"` // Distance in meters (optional)
